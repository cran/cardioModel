...
myModeling10 <- function(myData, AIC.k){
  ...
  # Mod01-E0+Slope
  tmp <- dfEmpty
  tmp$MODEL <- "01, model = e0 + slope, bsv = e0"
  ...
  #Mod02-E0+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "02, model = e0 + emax, bsv = e0"
  suppressWarnings(try({
  ...
  #Mod03-E0+SigEma
  tmp <- dfEmpty
  tmp$MODEL <- "03, model = e0 + sigmoidal emax, bsv = e0"
  suppressWarnings(try({
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "04, model = e0~fNTAFD + slope, bsv = e0~fNTAFD"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.slope.fun(Slo, Conc=EXPOSURE),
                    data=myData,
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "05, model = e0~fNTAFD + emax, bsv = e0~fNTAFD"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "06, model = e0~fNTAFD + sigmoidal emax, bsv = e0~fNTAFD"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod07-Cos24+Slope
  tmp <- dfEmpty
  tmp$MODEL <- "07, model = cosine 24 h period + slope, bsv = mean"
  suppressWarnings(try({
  ...
  ##Mod08-Cos24+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "08, model = cosine 24 h period + emax, bsv = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine24.fun(Mean, Amp1, Tpeak1, TOD=TOD)+
  ...
  ##Mod09-Cos24+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "09, model = cosine 24 h period + sigmoidal emax, bsv = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine24.fun(Mean, Amp1, Tpeak1, TOD=TOD)+
  ...
  ##Mod10-Cos12+Slope
  tmp <- dfEmpty
  tmp$MODEL <- "10, model = cosine 12 h period + slope, bsv = mean"
  suppressWarnings(try({
  ...
  ##Mod11-Cos12+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "11, model = cosine 12 h period + emax, bsv = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine12.fun(Mean, Amp2, Tpeak2, TOD=TOD)+
                      .my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod12-Cos12+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "12, model = cosine 12 h period + sigmoidal emax, bsv = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine12.fun(Mean, Amp2, Tpeak2, TOD=TOD)+
                      .my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod13-Dcos+Slope
  tmp <- dfEmpty
  tmp$MODEL <- "13, model = double cosine + slope, bsv = mean"
  suppressWarnings(try({
  ...
  ##Mod14-Dcos+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "14, model = double cosine + emax, bsv = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.dcosine.fun(Mean, Amp1, Tpeak1, Amp2, Tpeak2, TOD=TOD)+
                      .my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod15-Dcos+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "15, model = double cosine + sigmoidal emax, bsv = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.dcosine.fun(Mean, Amp1, Tpeak1, Amp2, Tpeak2, TOD=TOD)+
                      .my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  #Return results
  return(results.nlme.ETA.BL.noBOV)
}

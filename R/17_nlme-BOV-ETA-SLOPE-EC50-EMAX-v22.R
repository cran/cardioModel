###################################################################################
##############ETAs IN BASELINE & SLOPE/EC50/EMAX BOV for PERIOD######################
###################################################################################

myModeling17 <- function(myData, AIC.k){
  ...
  try(stop(""),TRUE)
  results.nlme.ETA.BL.SLO.EC50.EMAX.BOV <- data.frame()

  #################Estimated E0############################################

  ##Mod02-E0+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "91, model = e0 + emax, bsv = e0 +ec50 + emax, bov = e0"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
                    fixed=Int+Emax+EC50~1,
                    random=list(ID=Int+EC50+Emax~1, PERIOD=Int~1),
  ...
  try(stop(""),TRUE)
  results.nlme.ETA.BL.SLO.EC50.EMAX.BOV <- rbind(results.nlme.ETA.BL.SLO.EC50.EMAX.BOV, tmp)

  ##Mod03-E0+SigEma
  tmp <- dfEmpty
  tmp$MODEL <- "92, model = e0 + sigmoidal emax, bsv = e0 + ec50 + emax, bov = e0"
  suppressWarnings(try({
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "93, model = e0~fNTAFD + emax, bsv = e0~fNTAFD + ec50 + emax, bov = e0"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "94, model = e0~fNTAFD + sigmoidal emax, bsv = e0~fNTAFD + ec50 + emax, bov = e0"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod08-Cos24+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "95, model = cosine 24 h period + emax, bsv = mean + ec50 + emax, bov = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine24.fun(Mean, Amp1, Tpeak1, TOD=TOD)+
                    ...
                    random=list(ID=Mean+EC50+Emax~1, PERIOD=Mean~1),
  ...
  ##Mod09-Cos24+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "96, model = cosine 24 h period + sigmoidal emax, bsv = mean + ec50 + emax, bov = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine24.fun(Mean, Amp1, Tpeak1, TOD=TOD)+
  ...
  ##Mod11-Cos12+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "97, model = cosine 12 h period + emax, bsv = mean + ec50 + emax, bov = mean"
  suppressWarnings(try({
  ...
  ##Mod12-Cos12+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "98, model = cosine 12 h period + sigmoidal emax, bsv = mean + ec50 + emax, bov = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine12.fun(Mean, Amp2, Tpeak2, TOD=TOD)+
                      .my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod14-Dcos+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "99, model = double cosine + emax, bsv = mean + ec50 + emax, bov = mean"
  suppressWarnings(try({
  ...
  ##Mod15-Dcos+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "100, model = double cosine + sigmoidal emax, bsv = mean + ec50 + emax, bov = mean"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.dcosine.fun(Mean, Amp1, Tpeak1, Amp2, Tpeak2, TOD=TOD)+
                      .my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  mask.fake.error <- grep("try", results.nlme.ETA.BL.SLO.EC50.EMAX.BOV$ERROR.MESSAGE)
  results.nlme.ETA.BL.SLO.EC50.EMAX.BOV$ERROR.MESSAGE[mask.fake.error] <-
    "none"

  #Return results
  return(results.nlme.ETA.BL.SLO.EC50.EMAX.BOV)
}

###################################################################################
##############ETAs IN BASELINE & SLOPE/EC50/EMAX w/o BOV for PERD##################
###################################################################################

myModeling13 <- function(myData, AIC.k){
  ...
  try(stop(""),TRUE)
  results.nlme.ETA.BL.SLO.EC50.EMAX.noBOV <- data.frame()

  #################Estimated E0############################################

  ##Mod02-E0+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "41, model = e0 + emax, bsv = e0 +ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
                    fixed=Int+Emax+EC50~1,
                    random=list(ID=Int+EC50+Emax~1),
  ...
  try(stop(""),TRUE)
  results.nlme.ETA.BL.SLO.EC50.EMAX.noBOV <- rbind(results.nlme.ETA.BL.SLO.EC50.EMAX.noBOV, tmp)

  ##Mod03-E0+SigEma
  tmp <- dfEmpty
  tmp$MODEL <- "42, model = e0 + sigmoidal emax, bsv = e0 + ec50 + emax"
  suppressWarnings(try({
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "43, model = e0~fNTAFD + emax, bsv = e0~fNTAFD + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "44, model = e0~fNTAFD + sigmoidal emax, bsv = e0~fNTAFD + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod08-Cos24+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "45, model = cosine 24 h period + emax, bsv = mean + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine24.fun(Mean, Amp1, Tpeak1, TOD=TOD)+
                    ...
                    random=list(ID=Mean+EC50+Emax~1),
  ...
  ##Mod09-Cos24+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "46, model = cosine 24 h period + sigmoidal emax, bsv = mean + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine24.fun(Mean, Amp1, Tpeak1, TOD=TOD)+
  ...
  ##Mod11-Cos12+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "47, model = cosine 12 h period + emax, bsv = mean + ec50 + emax"
  suppressWarnings(try({
  ...
  ##Mod12-Cos12+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "48, model = cosine 12 h period + sigmoidal emax, bsv = mean + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.cosine12.fun(Mean, Amp2, Tpeak2, TOD=TOD)+
                      .my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  ##Mod14-Dcos+Emax
  tmp <- dfEmpty
  tmp$MODEL <- "49, model = double cosine + emax, bsv = mean + ec50 + emax"
  suppressWarnings(try({
  ...
  ##Mod15-Dcos+SigEmax
  tmp <- dfEmpty
  tmp$MODEL <- "50, model = double cosine + sigmoidal emax, bsv = mean + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.dcosine.fun(Mean, Amp1, Tpeak1, Amp2, Tpeak2, TOD=TOD)+
                      .my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  mask.fake.error <- grep("try", results.nlme.ETA.BL.SLO.EC50.EMAX.noBOV$ERROR.MESSAGE)
  results.nlme.ETA.BL.SLO.EC50.EMAX.noBOV$ERROR.MESSAGE[mask.fake.error] <-
    "none"

  #Return results
  return(results.nlme.ETA.BL.SLO.EC50.EMAX.noBOV)
}
